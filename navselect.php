...
$nnnavid = $navcalled->Fields("navid");
 while (!$nnnavid && ($nparent != $MX_top)) {
	...
	$navcalled = getnavs("  WHERE typelist=$nparent  ");
	$nnnavid = $navcalled->Fields("navid");
	} 
	}//end type
...
##GET ID NAV FILES ####
elseif ($_GET[id]){
$navcalled = getnavs("  WHERE typeid=$MM_type ");
$nparent =$MM_type;
$nnnavid = $navcalled->Fields("navid");
 while (!$nnnavid && ($nparent != $MX_top)) {
	$nparent=$obj->get_parent($nparent);
	$navcalled = getnavs("  WHERE typeid=$nparent  ");
	$nnnavid = $navcalled->Fields("navid");
	}
	if (!$nnnavid) {$navcalled = getnavs(" WHERE moduleid = $mod_id ");  
	$nnnavid  = $navcalled->Fields("navid");}
	}
...
if (!$nnnavid ) {$navcalled = getnavs(" WHERE moduleid = $mod_id ");
		$nnnavid  = $navcalled->Fields("navid");} 
if (!$nnnavid ) {$navcalled = getnavs(" WHERE moduleid = $modtemplate ");
		$nnnavid  = $navcalled->Fields("navid");}  
##cycle through list #####
$rowx_count=0;
    while (!$navcalled->EOF)   { 
