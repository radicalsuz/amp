...
//    $MM_editConnection = $MM__STRING;
    ...
    $MM_editRedirectUrl = "classlist_nav_edit.php";
    $MM_fieldsStr = "navid|value|position|value|moduleid|value|id|value";
    $MM_columnsStr = "navid|none,none,NULL|position|',none,''|classlist|none,none,NULL|id|none,none,NULL";
...
$mod_id = $HTTP_GET_VARS["id"];
 $navfiles=$dbcon->Execute("SELECT * FROM nav WHERE classlist=$id order by position asc") or DIE($dbcon->ErrorMsg());
    $navfiles_numRows=0;
   $navfiles__totalRows=$navfiles->RecordCount();
    $allnav=$dbcon->Execute("SELECT name, id FROM navtbl order by name asc") or DIE($dbcon->ErrorMsg());
   $allnav_numRows=0;
...
?>
                  ...
                  <?php echo  $allnav->Fields("name");?> </OPTION>
                   <?php
      $allnav->MoveNext();
      $allnav__index++;
    }
...
?>
    ...
    <input type="hidden" name="moduleid" value="<?php echo $navfiles->Fields("classlist"); ?>">
...
<a href="classlist_nav_add.php?id=<?php echo $HTTP_GET_VARS["id"]; ?>"> Insert Another Navigation File</a><br>
<a href="class.php?id=<?php echo $HTTP_GET_VARS["id"]; ?>">Back to Class</a>
