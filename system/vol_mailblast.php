...
function do_email_batch(&$offset, $chunksize) {
	...
	$sql = "SELECT vol_people.id, vol_people.email ";
	$sql .= " $passedsql ";
...
function get_count() {
	...
	# haven't tried this!!!
	$sql  = "SELECT  COUNT(DISTINCT vol_people.id)  ";
	$sql .= " $passedsql ";
...
function do_list_subscribers() {
	...
	## get mailing list ##
	$sql = "SELECT DISTINCT vol_people.id, vol_people.email ";
	$sql .= " $passedsql ";
