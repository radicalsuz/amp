...
function event_publish($ids) {
	...
	if (is_array($ids)) {
		foreach ($ids as $id) {
			$q = "update moduserdata set publish=1 where id=$id";
			#die($q);
...
function event_unpublish($ids) {
	...
	if (is_array($ids)) {
		foreach ($ids as $id) {
			$q = "update moduserdata set publish=0 where id=$id";
			#die($q);
			$dbcon->execute($q) or die($dbcon->errorMsg());
		}
		$qs = array(
...
if (($_GET["sort"]) == ("Organization")) {$sqlorder="ORDER BY Organization ASC";}
elseif (($_GET["sort"]) == ("LastName")) {$sqlorder="ORDER BY LastName ASC";}
...
if ($student) {$sqlstudent = " and field4 = 1";}
if  (($search) && ($search != "Search")) {
$sqlorg = " and $_GET[searchby] like '%$_GET[search]%' ";}


 $endorse=$dbcon->Execute("SELECT id, FirstName, LastName, Organization, publish  FROM moduserdata where modinid = $_GET[modin] $sqlorg    $sqlstudent  $sqlorder  Limit  $offset, $limit;" ) or DIE($dbcon->ErrorMsg()); 
 $sqlct= "SELECT  COUNT(DISTINCT id) FROM moduserdata where modinid = $_GET[modin] $sqlorg    $sqlstudent  $sqlorder ";
...
for(i=1; i<t; i++) document.forms[1][i].checked=document.forms[1][4].checked; 
		...
		<td><b><a href="moddata_list.php?modin=<?php echo $modin ?>&sort=id" class="intitle">id</a></b></td>
         <td><b><a href="moddata_list.php?modin=<?php echo $modin ?>&sort=LastName" class="intitle">Name</a></b></td>
		  <td><b><a href="moddata_list.php?modin=<?php echo $modin ?>&sort=Organization" class="intitle">Org</a></b></td>
...
?>
        <tr bgcolor="#CCCCCC"> 
		 ...
		 <td bgcolor="#ACACBF"><input type="checkbox" name="id[]" value="<?php echo $endorse->Fields("id")?>"></td>
          ...
          <td> <?php echo $endorse->Fields("FirstName")?>&nbsp;<?php echo $endorse->Fields("LastName")?> 
          </td>
          <td><?php echo $endorse->Fields("Organization")?> </td>
          <td><?php If (($endorse->Fields("publish")) == "1") { echo "live";} ?> 
          </td>
          <td><div align="right"><A HREF="moddata.php?modin=<?php echo $modin ?>&<?php echo "id=".$endorse->Fields("id") ?>">edit</A></div></td>
        </tr>
