...
elseif (($HTTP_GET_VARS["sort"]) == ("state")) {$sortvar="states.state"; }
...
if ($endorse== 1){$endosresql = " and endorse=1 ";}
else {$endosresql = "";}
if ($fpevent== 1){$fpeventsql = " and fpevent=1 ";}
else {$endosresql = "";}
if ($student== 1){$studentsql = " and student=1 ";}
else {$endosresql = "";}

   $currentevents=$dbcon->Execute("SELECT calendar.date, calendar.id, calendar.publish, calendar.event, states.state FROM calendar, states WHERE calendar.lstate=states.id and date $neworold Now()-1 $endosresql $fpeventsql $studentsql and repeat != 1 $sqldate2 $sqlarea $sqltype $sqlid ORDER BY ".$sortvar." asc") or DIE($dbcon->ErrorMsg());
...
?>
        <tr bgcolor="#CCCCCC"> 
		 <td bgcolor="#ACACBF"><input type="checkbox" name="id[]" value="<?php echo $currentevents->Fields("id")?>"></td>
          ...
          <td><?php echo $currentevents->Fields("state") ?></td>
...
elseif (($HTTP_GET_VARS["sort"]) == ("publish")) {$sortvar="calendar.publish"; }
   ...
   $currentevents=$dbcon->Execute("SELECT calendar.date, calendar.id, calendar.publish, calendar.event, states.state FROM calendar, states  WHERE calendar.lstate=states.id and repeat = 1 $sqldate2 $sqlarea $fpeventsql  $sqltype $sqlid $endosresql $studentsql ORDER BY ".$sortvar." asc") or DIE($dbcon->ErrorMsg());
   $currentevents_numRows=0;
   $currentevents__totalRows=$currentevents->RecordCount();
?><?php
   $Repeat1__numRows = -1;
