...
// amp_font_replace(".main_content_header .title .title_segment", "FrutigerLTStd-BoldCn", 13.5, "FFFFFF", "F0A800", 1, 1, 4 );
//    defaults are arial, white text, black background

function amp_font_replace( selector, face, size, color, background_color, shadow, transparent, kerning ) {
/*
...
if ( background_color ) url_vars.push('background_color=' + background_color);
if ( shadow )           url_vars.push('shadow=1');
if ( transparent)       url_vars.push('transparent=1');
if ( kerning)           url_vars.push('kerning=1');

replaceSelector( selector, ( base_url + url_vars.join( "&")), true );
var testURL = "/img/spacer.gif" ;

var doNotPrintImages = false;
var printerCSS = "print.css";
	...
	for(var i=0;i<items.length;i++)
	{
		...
		if(elements.length > 0) for(var j=0;j<elements.length;j++)
		{
			...
			for(var k=0;k<tokens.length;k++)
			{
				...
				var separator = (items[i].url.indexOf( "?")) ? "&" : "?";
                var text_padding = ( k == tokens.length-1 ) ? '' : ' ';
				var url = items[i].url + separator + "text="+escape(tokens[k]+text_padding)+"&selector="+escape(items[i].selector);
