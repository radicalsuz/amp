<?php

require_once( 'AMP/Content/Article/Public/List.php' );
require_once( 'AMP/Content/Article/Public/List/Grouped.php');
require_once( 'AMP/Content/Article/Public/List/Sections.php');
require_once( 'AMP/Content/Article/Public/List/Blog.php');
require_once( 'AMP/Content/Article/Public/List/Frontpage.php');
require_once( 'AMP/Content/Article/Public/Detail.php');
require_once( 'AMP/Content/Article/Public/Detail/Blog.php');
require_once( 'AMP/Content/Article/Public/Detail/Frontpage.php');
require_once( 'AMP/Content/Article/Public/Detail/Page.php');
require_once( 'AMP/Content/Article/Public/Detail/Pressrelease.php');
require_once( 'AMP/Content/Section/Public/List.php' );
require_once( 'AMP/Content/Section/Public/List/Contents.php' );
?>
