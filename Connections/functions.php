...
function buildheader() {
    ...
    global $MM_title, $MM_shortdesc, $MM_id, $_GET, $meta_description, $meta_content, $mod_name, $SiteName, $Web_url,$css;
    $htmlheader.= "<html>
    <head>
...
function pagination($count,$offset,$limit) {
	...
	if ($pages > 1) {
		...
		if ($pos) {$qs = substr($_SERVER['QUERY_STRING'],$pos);
			$qs= str_replace($qs,"", $_SERVER['QUERY_STRING']);}
		else {$qs = $_SERVER['QUERY_STRING'];}
		if ($_SERVER['QUERY_STRING']) { $qs= "?".$qs."&";}
		else {$qs="?";}
		while ($i != $pages) {
			if ($io == $offset) {echo "<strong>";} 
			echo "<a  href=\"".$_SERVER['PHP_SELF'].$qs."offset=";
			echo $io;
			echo "\">";
			echo ($i +1);
			echo "</a> ";
