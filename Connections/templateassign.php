<?php
/* 
this file loads the system vars and builds the template by working up the hierarchy unitll it finds an assigned template

 */
//set system vars from database
    $setvar=$dbcon->CacheExecute("SELECT template, emendorse, emfrom, metadescription, metacontent FROM sysvar WHERE id = 1") or DIE($dbcon->ErrorMsg());
...
if ( (!$template_id) && $MM_type != 1 && $MM_type )
  ...
  $gettemplate=$dbcon->CacheExecute("SELECT templateid, css FROM articletype WHERE id = $MM_type") or DIE($dbcon->ErrorMsg());
  ...
  $tparent= $MM_type;
	if ($css_inherit != NULL) {
		while ((!$template_id||!$css) && ($tparent != $MX_top)) {
			...
			$gettemplate=$dbcon->CacheExecute("SELECT templateid, css FROM articletype WHERE id = $tparent") or DIE($dbcon->ErrorMsg());
		...
		while (!$template_id && ($tparent != $MX_top)) {
			$tparent=$obj->get_parent($tparent);
			$gettemplate=$dbcon->CacheExecute("SELECT templateid FROM articletype WHERE id = $tparent") or DIE($dbcon->ErrorMsg());
			$template_id = $gettemplate->Fields("templateid");
		} 
	}
