...
$area = isset( $_GET['area']) && $_GET["area"] ? intval( $_GET['area']) : false; 
$clean_area = $dbcon->qstr( $area );

//set intro text
$sql=" select * from articles WHERE state  = " . $clean_area . " and publish=1 and  Class =  8 Limit 1 ";
$sql = $sqlsel.$sql.$sqlorder.$sqloffset;
$rheader=$dbcon->CacheExecute("$sql");
if ( $rheader ) {
    if ($rheader->Fields("id")){
        ...
        $rname=$dbcon->CacheExecute("select  title from region where id = $clean_area ;")or DIE($dbcon->ErrorMsg());
...
//events
if ($nonstateregion !=1) {
	$sqlarea = " and lstate = ".$clean_area ." " ;
}
else {  
	$sqlarea = " and region = ".$clean_area ." " ;
}
...
//action alerts
$sql=" WHERE state  = $clean_area  and publish=1 and  Class =  5 ";
$sql = $sqlsel.$sql.$sqlorder.$sqloffset;
$list=$dbcon->CacheExecute("$sql");
if ($list && $list->Fields("id")){
...
if ($nonstateregion !=1) {
  	$sqlarea = " and  moduserdata.State = ".$clean_area." " ;
}
else {  
	$sqlarea = " and  moduserdata.region = ".$clean_area." " ;
}
$groups=$dbcon->CacheExecute("SELECT moduserdata.*, states.state FROM moduserdata, states  WHERE moduserdata.publish = '1' and moduserdata.modinid=2  and states.id=moduserdata.State $sqlarea   ORDER BY moduserdata.Organization ASC ") ;
if ($groups && $groups->Fields("id")){
...
//content
$sql="WHERE state = $clean_area and publish=1 and  (class !=2 && class !=8 && class !=9 && class !=3 && class !=5)  ";
$sql = $sqlsel.$sql.$sqlorder.$sqloffset;
$list=$dbcon->CacheExecute("$sql");
if ($list && $list->Fields("id")){
...
//news 
$sql="WHERE state = $clean_area and publish=1 and  Class =  3 ";
$sql = $sqlsel.$sql.$sqlorder.$sqloffset;
$list=$dbcon->CacheExecute("$sql");
if ($list && $list->Fields("id")){
...
//press releases
$sql="WHERE state = $clean_area and publish=1 and  Class =  10" ;
$sql = $sqlsel.$sql.$sqlorder.$sqloffset;
$list=$dbcon->CacheExecute("$sql");
if ($list && $list->Fields("id")){
