...
class CalendarPlugin_TableHTML_Output extends CalendarPlugin {
    ...
    var $options= array( 
        ...
        'display_fields'=>array('value'=>'id,Scheduled,Event,City,State,Status'),
        'display_format'=>array('value'=>'table_format'),
        'form_name'=>array('value'=>'calendar_list'),
        'editlink'=>array('value'=>'calendar_gxedit.php')
        );
    var $html_rowcount=0;
    ...
    function CalendarPlugin_TableHTML_Output (&$calendar, $options=null, $instance=null) {   
    ...
    function execute ($options=null) {
        ...
        if (!($dataset=$this->calendar->results())) return false;
        ...
        if ($this->calendar->admin) $output.=$this->select_script();
    ...
    function _register_options_dynamic(){
        if ($this->calendar->admin) { //make editing permission available
            ...
            $this->list_row_edit=sprintf($this->options['list_item_start']['value'],'editlink').
                "<a href=\"".$this->options['editlink']['value']."?calid=%s\">edit</a>".$this->options['list_item_end']['value'];
        }
    }
    ...
    function _register_fields_dynamic(){
        ...
        foreach ($fields as $fname=>$fdef) {
            $this->calendar->fields[$fname]=$fdef;
        }*/
    }

    function column_headers() {
        foreach ($this->display_fieldset as $key) {
            if (isset($this->calendar->plugins['Sort']['AMP'])) {
                $sort_obj=&$this->calendar->plugins['Sort']['AMP'];
                $key=$sort_obj->makelink($key);
            }
            $list_html_headers.=sprintf($this->options['list_html_header_template']['value'], $key);
        }
        if ($this->calendar->admin) { //include columns headers for select and edit cols
    ...
    function table_format($current_row) {
        ...
        foreach($this->display_fieldset as $key) {
            ...
            $list_row.=sprintf($this->options['list_item_start']['value'], $key).
                        $current_row[$key].$this->options['list_item_end']['value'];
        }
