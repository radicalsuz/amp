...
/*****
 *
 * AMP UserDataModule HTML_List builder Plugin
 *
 * Creates an HTML_List object based on the contents of
 * an UDM object.
 *
 *****/
...
class UserDataPlugin_UserlistHTML_Output extends UserDataPlugin {

    // A little bit of friendly information about the plugin.
    var $short_name  = 'List Page';
    var $long_name   = 'Administrative Listing of Users';
    var $description = 'Use this to set options for the list which appears on the admin interface';
    //List Vars
    ...
    function UserDataPlugin_UserlistHTML_Output ( &$udm, $plugin_instance=null ) {
    ...
    function authval () {
        print ($this->udm->authorized?"yes<BR>":"No<BR>");
    }
...
//UDM INSTANCE CONTROL PANEL
	...
	function list_include_modin_box(&$udm, &$options) {
		...
		$modinbox.="</b><BR><input name=\"UDM_include_all_modins\" type=\"checkbox\" value=\"1\" $is_selected>
                List users from all sources<BR><a href=\"javascript: void(0);\" onclick=\"change_any('UDM_list_modules')\">Select sources</a><br>
                <div id=\"UDM_list_modules\" style=\"display: $modlist_style;\">Also include:<BR>
                <SELECT MULTIPLE name=\"UDM_include_modins[]\" size=\"6\">".GetModsOptions($udm, $options)."</select><br>
