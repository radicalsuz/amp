...
class UserData_Plugin_Save_AMP extends UserData_Plugin_Save {

    function __doSave ( $frmFieldValues ) {

        $sql = ($this->dataUid) ? $this->__doUpdate( $frmFieldValues ) :
                                  $this->__doInsert( $frmFieldValues );

        $rs = $this->dbcon->Execute( $sql ) or
                    die( "Unable to save request data: " . $dbcon->ErrorMsg() );

        if ($rs) {

            $this->udm->showForm = false;

            $this->dataUid = $dbcon->Insert_ID();
            $this->udm->uid = $this->dataUid;
    ...
    function __doUpdate ( $frmFieldValues ) {
    ...
    function __doInsert ( $frmFieldValues ) {
