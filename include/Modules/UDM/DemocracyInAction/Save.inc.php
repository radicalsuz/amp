...
class UserDataPlugin_Save_DemocracyInAction extends UserDataPlugin_Save {
    var $options = array(
        'node' => array(
            'type'=>'text',
            'size'=>'15',
    ...
    function _init_api( $options = array( )){
        if ( isset( $this->_dia_api )) return;

        if(!( isset($options[ 'orgKey' ]) && $options['orgKey']) && defined('DIA_API_NODE')) {
            $options['orgKey'] = DIA_API_NODE;
        }
        ...
        $this->_dia_api =& new DemocracyInAction_API($options['node'], 
                                                     $options['user'],
                                                     $options['password']);
    }
    ...
    function addLinkedMapping( $supporter_id, $mapped_data, $table ){
        ...
        return $this->_dia_api->save( $table, $mapped_data );
    }
