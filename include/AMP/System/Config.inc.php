...
/*
$system_values = parse_ini_file( 'Config/system.inc', true );
foreach( $system_values as $chunk_label => $items ) {
    foreach( $items as $label => $value ) {
        $constant_name = str_replace( ' ', '_', strtoupper( 'AMP ' . $chunk_label . ' ' . $label ));
        if ( !defined( $constant_name )) {
            define( $constant_name, $value );
            trigger_error( $constant_name . ' :: ' . $value );
        }
    }
}
...
if ( !defined( 'AMP_NULL_DATETIME_VALUE_DB')) define('AMP_NULL_DATETIME_VALUE_DB', '0000-00-00 00:00:00' );
...
if ( !defined( 'AMP_FUTURE_DATETIME_VALUE_UNIX')) define('AMP_FUTURE_DATETIME_VALUE_UNIX', '2100-12-31 16:33:26' );
 ...
 *  GLOBAL settings
 *
define( "AMP_SYSTEM_VERSION_ID", '3.7.2');
