...
function display_quote($type=NULL) {
	...
	$Q=$dbcon->CacheExecute("Select * from quotes where $where_date $where_type publish =1 limit 1");
	if (!$Q->Fields("quote")) {
			$Q=$dbcon->CacheExecute("Select * from quotes where $where_type publish =1 limit 1");
	}
