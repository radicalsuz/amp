...
if (isset($_GET['linktype']) && $_GET['linktype']) {
	$sql = "SELECT l. * , t.name FROM links l, linksreltype r, linktype t WHERE l.id = r.linkid AND l.linktype = t.id AND r.typeid=" . $dbcon->qstr($_GET['linktype']) . " AND l.publish = '1' order by t.name asc, if( ( isnull( l.linkorder ) or l.linkorder=0), '".AMP_SORT_END."', l.linkorder ), l.linkname asc";
} else { 
	$sql = "SELECT l. * , t.name FROM links l, linktype t WHERE  l.linktype = t.id  AND l.publish = '1' AND t.publish = '1' order by t.name asc, if( ( isnull( l.linkorder ) or l.linkorder=0), '".AMP_SORT_END."', l.linkorder), l.linkname asc";
}
