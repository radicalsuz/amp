...
class ArticleDisplay_PressRelease extends ArticleDisplay_News {
    ...
    function _HTML_date( $date ) {
        ...
        return $this->_HTML_inSpan( "For Immediate Release:&nbsp;", "bodygreystrong" ) .
                $this->_HTML_inSpan( DoDate( $date, 'F jS, Y'), "bodygrey") . $this->_HTML_newline();
    }

    function _HTML_contact( $contact ) {
        if (!$contact) return false;
        return  '<table border="0" cellpadding="0" cellspacing="0"><tr>'.
                $this->_HTML_inTD( $this->_HTML_inSpan( 'Contact:&nbsp;', 'bodygreystrong' ), array('valign'=>'top')).
                $this->_HTML_inTD( '&nbsp;', array('valign'=>'top')).
                $this->_HTML_inTD( $this->_HTML_inSpan( converttext( $contact ), 'bodygrey' ) .$this->_HTML_newline() , array('valign'=>'top')).
                $this->_HTML_endTable();
    }
