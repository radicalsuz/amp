...
class Article_ListForm extends AMP_System_List_Form {
    ...
    function _init_default_sort( ){
        $this->_sort_default = array( 
            ...
            'pageorder' => 'if ( !isnull( pageorder ) AND pageorder != "" AND pageorder !=0, pageorder, '.AMP_SORT_MAX.')',
            'date' => 'if ( !isnull( date ), date, "0000-00-00" ) DESC',
            'id' => 'id DESC'
        );
        $this->_sort_translations_sql['order'] = 'if ( !isnull( pageorder ) AND pageorder != "" AND pageorder !=0 , pageorder, '. AMP_SORT_MAX.')';
    }
