...
class AMP_Authentication_Handler {
    ...
    function check_authen_credentials() {
        ...
        if (isset($_REQUEST['username']) || isset($PHP_AUTH_USER))
    ...
    function set_cookie () {
        ...
        if (setcookie( 'AMPLoginCredentials', "$hash:$c_user:$c_perm:$c_userid", $now + $this->timeout, $c_domain )) {
    ...
    function check_coookie ( $cookie ) {

        $dbcon = $this->dbcon;

        list( $cookie['hash'], $cookie['user'], $cookie['permission'], $cookie['userid'] )
            = explode( ":", $_COOKIE['AMPLoginCredentials'] );
        ...
        if ($authdata) {
            ...
            if ($this->cookie_still_valid( $authdata['last_time'] )) {
                ...
                $this->invalidate_cookie('Expired Session. Please re-enter your login information.');
                return false;

           }
        }

        $this->invalidate_cookie('Invalid Credentials.', true);
        return false;
    ...
    function invalidate_cookie ( $message, $attack = false ) {

        $this->error_message = $message;

        $c_domain = preg_replace( "/([^\.]*)\.([^\.]*)$/", "/.\$1.\$2/", $_SERVER['SERVER_NAME'] );
        return setcookie( 'AMPLoginCredentials', '*', time() - 86400, $c_domain );
